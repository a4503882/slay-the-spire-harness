...
public class CommunicationMod implements PostInitializeSubscriber, PostUpdateSubscriber, PostDungeonUpdateSubscriber, PreUpdateSubscriber, OnStateChangeSubscriber {
    ...
    private static final String HARNESS_PYTHON_ENV = "STS_HARNESS_PYTHON";
    private static final String HARNESS_WORKER_ENV = "STS_HARNESS_WORKER";
    private static final String HARNESS_AUTOSTART_ENV = "STS_HARNESS_AUTOSTART";
    ...
    private static String[] getSubprocessCommand() {
        String harnessPython = System.getenv(HARNESS_PYTHON_ENV);
        String harnessWorker = System.getenv(HARNESS_WORKER_ENV);
        if (harnessPython != null && !harnessPython.trim().isEmpty()
                && harnessWorker != null && !harnessWorker.trim().isEmpty()) {
            return new String[] {harnessPython.trim(), "-u", harnessWorker.trim()};
        }
        if (communicationConfig == null) {
            return new String[0];
        }
    ...
    private static String getSubprocessCommandString() {
        ...
        if (harnessPython != null && !harnessPython.trim().isEmpty()
                && harnessWorker != null && !harnessWorker.trim().isEmpty()) {
            return harnessPython.trim() + " -u " + harnessWorker.trim();
        }
        if (communicationConfig == null) {
            return "";
        }
    ...
    private static boolean getRunOnGameStartOption() {
        if ("1".equals(System.getenv(HARNESS_AUTOSTART_ENV))) {
            return true;
        }
        if (communicationConfig == null) {
            return false;
            ...
            if(message == null) {
                ...
                if (!"ready".equalsIgnoreCase(message.trim())) {
                    ...
                    logger.error(String.format(
                            "External process sent an invalid initialization signal: %s", message));
                    return false;
                }
                logger.info("Received ready signal from external process.");
